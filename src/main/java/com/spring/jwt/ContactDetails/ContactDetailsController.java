 ...
 * Implements secure endpoints with proper authorization and comprehensive API
 * documentation.
 * All user operations are scoped to the authenticated user's data to prevent
...
public class ContactDetailsController {
    ...
    @Loggable(action = "CREATE_CONTACT_DETAILS")
    @Operation(summary = "Create contact details", description = "Create contact details for the authenticated user")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "201", description = "Contact details created successfully", content = @Content(schema = @Schema(implementation = ContactDetailsResponse.class))),
    ...
    @PreAuthorize("hasRole('USER')")
    @RateLimiter(name = "contactDetailsApi")
    @Operation(summary = "Get current user's contact details", description = "Retrieve contact details for the authenticated user")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Contact details retrieved successfully", content = @Content(schema = @Schema(implementation = ContactDetailsResponse.class))),
    ...
    @PreAuthorize("hasRole('USER')")
    @RateLimiter(name = "contactDetailsApi")
    @Loggable(action = "UPDATE_CONTACT_DETAILS")
    @Operation(summary = "Update current user's contact details", description = "Update contact details for the authenticated user")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Contact details updated successfully", content = @Content(schema = @Schema(implementation = ContactDetailsResponse.class))),
            @ApiResponse(responseCode = "400", description = "Invalid input data"),
            @ApiResponse(responseCode = "404", description = "Contact details not found"),
    ...
    @PreAuthorize("hasRole('USER')")
    @RateLimiter(name = "contactDetailsApi")
    @Loggable(action = "DELETE_CONTACT_DETAILS")
    @Operation(summary = "Delete current user's contact details", description = "Soft delete contact details for the authenticated user")
    @ApiResponses(value = {
    ...
    @Operation(summary = "Verify mobile number", description = "Verify mobile number with verification code")
    @ApiResponses(value = {
    ...
    public ResponseEntity<ResponseDto<Boolean>> verifyMobileNumber(
            @Parameter(description = "Verification code", required = true) @RequestParam String verificationCode) {
    ...
    @PreAuthorize("hasRole('USER')")
    @RateLimiter(name = "verificationApi")
    @Operation(summary = "Verify email address", description = "Verify email address with verification code")
    @ApiResponses(value = {
    ...
    @PreAuthorize("hasRole('USER')")
    @RateLimiter(name = "verificationApi")
    @Operation(summary = "Send mobile verification code", description = "Send verification code to mobile number")
    @ApiResponses(value = {
    ...
    @PreAuthorize("hasRole('USER')")
    @RateLimiter(name = "verificationApi")
    @Operation(summary = "Send email verification code", description = "Send verification code to email address")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Verification code sent successfully"),
    ...
    @Operation(summary = "Get contact details by user ID", description = "Retrieve contact details for a specific user (Admin only)")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Contact details retrieved successfully", content = @Content(schema = @Schema(implementation = ContactDetailsResponse.class))),
            @ApiResponse(responseCode = "404", description = "Contact details not found"),
            @ApiResponse(responseCode = "401", description = "Unauthorized - Authentication required"),
    ...
    @GetMapping
    @PreAuthorize("hasRole('ADMIN')")
    @Operation(summary = "Get all contact details", description = "Retrieve all contact details with pagination (Admin only)")
    @ApiResponses(value = {
    ...
    public ResponseEntity<ResponseDto<Page<ContactDetailsResponse>>> getAllContactDetails(
            ...
            @Parameter(description = "Sort direction", example = "DESC") @RequestParam(defaultValue = "DESC") Sort.Direction sortDir) {
    ...
    @PreAuthorize("hasRole('ADMIN')")
    @Operation(summary = "Search contact details by location", description = "Search contact details by location criteria (Admin only)")
    @ApiResponses(value = {
    ...
    public ResponseEntity<ResponseDto<Page<ContactDetailsResponse>>> searchByLocation(
            @Parameter(description = "City filter", example = "Bangalore") @RequestParam(required = false) String city,

            @Parameter(description = "State filter", example = "Karnataka") @RequestParam(required = false) String state,

            @Parameter(description = "Country filter", example = "India") @RequestParam(required = false) String country,

            @Parameter(description = "Page number (0-based)", example = "0") @RequestParam(defaultValue = "0") @Min(0) int page,

            @Parameter(description = "Page size", example = "20") @RequestParam(defaultValue = "20") @Min(1) @Max(100) int size,

            @Parameter(description = "Sort field", example = "city") @RequestParam(defaultValue = "city") String sortBy,

            @Parameter(description = "Sort direction", example = "ASC") @RequestParam(defaultValue = "ASC") Sort.Direction sortDir) {
    ...
    @PreAuthorize("hasRole('ADMIN')")
    @Operation(summary = "Search contact details by verification status", description = "Search contact details by verification status (Admin only)")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Search results retrieved successfully"),
            @ApiResponse(responseCode = "401", description = "Unauthorized - Authentication required"),
            @ApiResponse(responseCode = "403", description = "Forbidden - Admin role required")
    ...
    public ResponseEntity<ResponseDto<Page<ContactDetailsResponse>>> searchByVerificationStatus(
            @Parameter(description = "Mobile verification status", example = "true") @RequestParam(required = false) Boolean mobileVerified,

            @Parameter(description = "Email verification status", example = "false") @RequestParam(required = false) Boolean emailVerified,

            @Parameter(description = "Page number (0-based)", example = "0") @RequestParam(defaultValue = "0") @Min(0) int page,

            @Parameter(description = "Page size", example = "20") @RequestParam(defaultValue = "20") @Min(1) @Max(100) int size,

            @Parameter(description = "Sort field", example = "isVerifiedMobile") @RequestParam(defaultValue = "isVerifiedMobile") String sortBy,
    ...
    @PreAuthorize("hasRole('ADMIN')")
    @Operation(summary = "Get contact details statistics", description = "Retrieve statistical information about contact details data (Admin only)")
    @ApiResponses(value = {
